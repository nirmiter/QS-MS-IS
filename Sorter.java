...
public abstract class Sorter
{
  ...
  {
	  WordList sorted = toSort.clone();
	  CountingComparator<String> s = new CountingComparator<>(comp);
	 for(int i = 0; i < totalToSort / toSort.length(); i++) {
		 sorted = toSort.clone();
	  	if(this.totalWordsSorted <= totalToSort) {
		  long startTime = System.currentTimeMillis();
	  		
		  sort(sorted,s);
		  long endTime = System.currentTimeMillis();
		  this.totalSortingTime += (endTime - startTime);
		  this.totalWordsSorted += toSort.length();
	  		}
  }
   try {
	PrintWriter write = new PrintWriter(getClass().getSimpleName() +".txt");
    for (int i = 0; i < sorted.length(); i++) {
    	write.println(sorted.get(i));
    }
    write.close();
   }
    ...
    this.totalComparisons = s.getCount();
  }
